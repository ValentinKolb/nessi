...
const WEB_SKILL_CODE = String.raw`export default function create(api) {
...
const SURVEY_SKILL_CODE = String.raw`export default function create(api) {
  ...
  return cli({ name: "survey", description: "Ask structured single-choice questions" })
    .sub({
      name: "ask",
      usage: "ask --json '{\"title\":\"...\",\"questions\":[{\"question\":\"...\",\"options\":[\"A\",\"B\"]}]}'",
      async handler(args) {
        const opts = parseArgs(args);
        ...
        if (json) {
          ...
          if (!question || !optionsRaw) {
            return err("Usage: survey ask <question> --options \"A|B|C\" [--title \"...\"] OR survey ask --json '{...}'");
          }
          payload = {
        ...
        if (!normalized) {
          return err("Error: invalid survey payload.");
        }
...
export const BUILTIN_SKILL_CODES: Record<string, string> = {
  survey: SURVEY_SKILL_CODE,
  web: WEB_SKILL_CODE,
};
