...
export const ContextOverflowBlock = (props: {
  ...
  const contextWindow = () => props.block.contextWindow;
  const ratio = () => props.block.overflowRatio;
      ...
      <div class="flex items-start gap-2">
        ...
        <div class="flex-1 min-w-0 space-y-1">
          ...
          <p class="text-gh-fg-muted leading-relaxed">
            {ratio() && contextWindow()
              ? `The conversation is ~${formatTokens(Math.round(ratio()! * contextWindow()!))} tokens, but this model supports max. ${formatTokens(contextWindow()!)}. `
